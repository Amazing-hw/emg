...
def train(
    ...
    # --- Load pre-trained encoder weights if configured ---
    pretrained_ckpt = config.get("pretrained_encoder_ckpt", None)
    if pretrained_ckpt and hasattr(module.network, 'load_encoder_from_ckpt'):
        logger.info(f"Loading pre-trained encoder from {pretrained_ckpt}")
        module.network.load_encoder_from_ckpt(pretrained_ckpt)
    # ------------------------------------------------------

    logger.info("Instantiating LightningDataModule")
    # Handle multi-task mixed data module
    if config.get("joint_data_location") is not None:
        from emg_transfer.multitask_data import MixedEmgDataModule
        datamodule = MixedEmgDataModule(
            gesture_data_location=config.get("data_location", "D:/emg/emg_nature/emg_data"),
            joint_data_location=config.get("joint_data_location"),
            window_length=16000,
            stride=16000,
            batch_size=4,
            joint_batch_size=4,
            num_workers=0,
            joint_data_fraction=config.get("joint_data_fraction", 0.5),
            gesture_split_csv=config.get("gesture_split_csv", None),
        )
    else:
    ...
    if config.train:
        ...
        module_and_results = _load_and_report_best_checkpoint(
            trainer=trainer, module=module, results=results, logger=logger,
        )
        ...
        if pl.utilities.rank_zero_only.rank == 0:
            ...
            if config.get("joint_data_location") is not None:
                ...
                datamodule = MixedEmgDataModule(
                    gesture_data_location=config.get("data_location", "D:/emg/emg_nature/emg_data"),
                    joint_data_location=config.get("joint_data_location"),
                    window_length=16000, stride=16000, batch_size=4,
                    joint_batch_size=4, num_workers=0,
                    joint_data_fraction=config.get("joint_data_fraction", 0.5),
                    gesture_split_csv=config.get("gesture_split_csv", None),
                )
            else:
            ...
            results = _run_validate_and_test(
                module=module, datamodule=datamodule, results=results,
                logger=logger, accelerator=accelerator, config=config,
            )
...
def _load_and_report_best_checkpoint(trainer, module, results, logger):
...
def _run_validate_and_test(
    module, datamodule, results, logger, accelerator, config,
    evaluate_validation_set=True, evaluate_test_set=True,
):
...
@hydra.main(
    config_path="../config",
    config_name="discrete_gestures_transfer",
    version_base="1.1",
)
