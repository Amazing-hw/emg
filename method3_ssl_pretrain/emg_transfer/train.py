...
def train(
    ...
    if pretrained_ckpt:
        if hasattr(module.network, 'load_encoder_from_ckpt'):
            logger.info(f"Loading pre-trained encoder from {pretrained_ckpt}")
            module.network.load_encoder_from_ckpt(pretrained_ckpt)
        elif hasattr(module.network, 'load_pretrained'):
            logger.info(f"Loading SSL pre-trained weights from {pretrained_ckpt}")
            module.network.load_pretrained(pretrained_ckpt)
        else:
            logger.warning(
                f"Pre-trained checkpoint specified but network has no loading method. "
                f"Available methods: {[m for m in dir(module.network) if 'load' in m.lower()]}"
            )
    # ------------------------------------------------------
